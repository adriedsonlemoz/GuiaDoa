...
O painel Admin possui um Alliance Tracker que lê screenshots de **Poder**, **Última Conexão** e **Data de Entrada**. A leitura usa OCR local com Tesseract.js, ROI, segunda passada adaptativa, confiança por linha e um resolvedor próprio que cruza membros/aliases conhecidos, histórico e correções já confirmadas. Na Beta 2.42, a Groq foi removida completamente do Alliance Tracker: não existe chamada de IA externa nem fallback de modelo neste módulo. Falhas estruturais viram casos de revisão manual e não interrompem o lote; as próximas imagens continuam sendo processadas. A leitura permanece NDJSON progressiva, sequencial, retomável e com checkpoints locais.

Enquanto o lote está ativo, os screenshots ficam apenas em armazenamento temporário do backend para permitir retomada após refresh/reconexão. Duplicados são ignorados por hash; o OCR da imagem atual pode ser retomado por checkpoint sem nova leitura, e resultados concluídos são preservados. Os arquivos temporários são apagados ao concluir/cancelar e lotes abandonados expiram. Somente os dados revisados são persistidos no MongoDB. A primeira captura marcada como lista completa vira baseline; capturas completas posteriores detectam entrada, saída e retorno, com limite de 120 membros.
