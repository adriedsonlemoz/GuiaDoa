...
O painel Admin possui um Alliance Tracker que lê screenshots de **Poder**, **Última Conexão** e **Data de Entrada**. A primeira tentativa usa OCR local com Tesseract.js no próprio backend, ROI para separar cabeçalho/tabela, segunda passada adaptativa e confiança por linha. Linhas seguras são preservadas; somente capturas com exceções usam a Groq como fallback. Nesse fallback, `GROQ_API_KEY` habilita a leitura visual, `GROQ_VISION_MODEL` define o modelo principal (padrão: `qwen/qwen3.6-27b`) e `GROQ_VISION_FALLBACK_MODEL` define a alternativa automática. A leitura no Admin usa NDJSON progressivo, processa uma imagem por vez e trata HTTP 429 da Groq com `Retry-After` + retry/backoff automático.
