...
function renderCampanha(lista, counts) {
  ...
  document.getElementById('content').innerHTML = `
    ...
    <div class="card">
      ...
      <div class="card-body">
        <p class="admin-help-copy">Dados oficiais, recompensas e estratégias ficam separados. Recompensas podem usar imagem local; quantidade é opcional porque o drop pode variar por reino. Itens sem nome confirmado podem usar apenas R1, R2… até a identificação no jogo.</p>
...
function garantirModalCampanha() {
  ...
  modal.innerHTML = `<div class="modal mc-modal"><div class="modal-header"><h3 id="mc-title">Novo registro</h3><button class="modal-close" onclick="fecharModal('modal-campanha')">✕</button></div>
    <div class="modal-body">
      ...
      <div class="mc-editor-block"><div class="mc-editor-head"><strong>◇ Recompensas possíveis</strong><button class="btn btn-ghost btn-sm" type="button" onclick="mcAddReward()">＋ Recompensa</button></div><div class="grid2"><div class="field"><label>Status da lista</label><select id="mc-rewards-status"><option value="pendente">Pendente</option><option value="parcial">Parcial</option><option value="confirmado">Confirmado</option></select></div><div class="field"><label>Tags do registro</label><input id="mc-tags" placeholder="campo, lago, obtenção-dragões"></div></div><div id="mc-rewards"></div><p class="admin-help-copy">Use “Confirmado” inclusive quando o nível realmente não possuir recompensas. Metadados semânticos dos itens ficam disponíveis para futuros tutoriais automáticos.</p></div>
...
function mcAddReward(data={}) { const row=document.createElement('div'); row.className='mc-reward-editor'; row.innerHTML=`<div class="mc-row mc-reward-row"><input class="mc-reward-symbol" placeholder="R1" value="${esc(data.simbolo||'')}"><input class="mc-reward-name" placeholder="Nome PT-BR (opcional)" value="${esc(data.nome||'')}"><input class="mc-reward-qty" type="number" min="0" placeholder="Qtd. (opcional)" value="${data.quantidade==null?'':Number(data.quantidade)}"><button type="button" class="btn btn-red btn-sm" onclick="this.closest('.mc-reward-editor').remove()">✕</button></div><div class="mc-row mc-reward-row-secondary"><input class="mc-reward-name-en" placeholder="Name EN-US" value="${esc(data.i18n?.['en-US']?.nome||'')}"><input class="mc-reward-code" placeholder="Código estável" value="${esc(data.codigo||'')}"><label class="mc-inline-check"><input class="mc-reward-confirmed" type="checkbox" ${data.nomeConfirmado?'checked':''}> nome confirmado</label></div><div class="mc-row mc-reward-row-secondary"><input class="mc-reward-category" placeholder="Categoria semântica" value="${esc(data.categoria||'')}"><input class="mc-reward-purpose" placeholder="Finalidade" value="${esc(data.finalidade||'')}"><input class="mc-reward-related" placeholder="Relacionado a" value="${esc(data.relacionadoA||'')}"></div><div class="grid2"><div class="field"><label>Imagem do item</label><input class="mc-reward-image" placeholder="/assets/items/..." value="${esc(data.imagem||'')}"></div><div class="field"><label>Tags do item</label><input class="mc-reward-tags" placeholder="dragão, obtenção" value="${esc((data.tags||[]).join(', '))}"></div></div></div>`; document.getElementById('mc-rewards').appendChild(row); }
const mcLines = value => String(value || '').split('\n').map(x=>x.trim()).filter(Boolean);
const mcJoin = value => Array.isArray(value) ? value.join('\n') : '';
const mcTags = value => String(value || '').split(',').map(x=>x.trim()).filter(Boolean);
...
function abrirModalCampanha(item) {
  ...
  document.getElementById('mc-rewards-status').value=item?.recompensasStatus||((item?.recompensas||[]).length?'parcial':'pendente'); document.getElementById('mc-tags').value=(item?.tags||[]).join(', ');
...
async function salvarCampanha() {
  ...
  const recompensas=[...document.querySelectorAll('.mc-reward-editor')].map((box,i)=>({codigo:box.querySelector('.mc-reward-code').value.trim(),simbolo:box.querySelector('.mc-reward-symbol').value.trim()||`R${i+1}`,nome:box.querySelector('.mc-reward-name').value.trim(),imagem:box.querySelector('.mc-reward-image').value.trim(),quantidade:box.querySelector('.mc-reward-qty').value===''?null:Number(box.querySelector('.mc-reward-qty').value),nomeConfirmado:box.querySelector('.mc-reward-confirmed').checked,categoria:box.querySelector('.mc-reward-category').value.trim(),finalidade:box.querySelector('.mc-reward-purpose').value.trim(),relacionadoA:box.querySelector('.mc-reward-related').value.trim(),tags:mcTags(box.querySelector('.mc-reward-tags').value),i18n:{'en-US':{nome:box.querySelector('.mc-reward-name-en').value.trim()}}}));
