...
function mcAddGuide(data={}) {
  ...
  box.innerHTML=`<input class="mc-guide-code" type="hidden" value="${esc(data.codigo||'')}"><div class="mc-editor-head"><strong>${esc(data.titulo||'Novo método')}</strong><button type="button" class="btn btn-red btn-sm" onclick="this.closest('.mc-guide-editor').remove()">✕</button></div>
    <div class="grid2"><div class="field"><label>Título PT-BR *</label><input class="mc-guide-title" value="${esc(data.titulo||'')}"></div><div class="field"><label>Status</label><select class="mc-guide-status"><option value="validacao" ${data.status!=='confirmado'?'selected':''}>Em validação</option><option value="confirmado" ${data.status==='confirmado'?'selected':''}>Confirmado</option></select></div></div>
    ...
    <div class="i18n-section"><div class="i18n-title">🌐 English <span>attack method</span></div><div class="field"><label>Title</label><input class="mc-guide-title-en" value="${esc(en.titulo||'')}"></div><div class="field"><label>Summary</label><textarea class="mc-guide-summary-en">${esc(en.resumo||'')}</textarea></div><div class="field"><label>Steps (1 per line)</label><textarea class="mc-guide-steps-en">${esc(mcJoin(en.passos))}</textarea></div><div class="field"><label>Notes</label><textarea class="mc-guide-notes-en">${esc(en.observacoes||'')}</textarea></div></div>`;
