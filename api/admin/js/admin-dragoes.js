...
function renderDragoes(d) {
  document.getElementById('content').innerHTML = `
    <div class="stats-row">
      ...
      <div class="stat-box"><div class="stat-val">${d.dragoes.reduce((s,dr)=>s+(dr.niveis?.length||0),0)}</div><div class="stat-lbl">Níveis cadastrados</div></div>
    </div>
    <div class="card">
          ...
          <div class="tabela-wrap">
            <table>
              ...
              <tbody>
                ${d.dragoes.map(dr => `
                  <tr>
                    <td style="text-align:center;font-size:1.4rem">${esc(dr.emojiDragao || '🐉')}</td>
                    <td>
