...
var EVENTO_TORNEIO_LINKS=[['','— Sem vínculo —'],['aceleracoes','Acelerações'],['general','Generais'],['treino_tropa','Recrutamento de tropas'],['matar_tropas','Eliminação de tropas'],['aprimoramento_tropa','Aprimoramento de tropa'],['habilidade_dragao','Habilidade de dragão'],['treinamento_dragao','Treinamento do dragão'],['pocoes_antigas','Poções antigas'],['talisma','Talismã'],['evolucao_tropas','Evolução de tropas'],['poder','Poder'],['alianca','Aliança']];
function eventoTorneioOptions(selected){return EVENTO_TORNEIO_LINKS.map(([id,label])=>`<option value="${esc(id)}" ${String(selected||'')===id?'selected':''}>${esc(label)}</option>`).join('');}
