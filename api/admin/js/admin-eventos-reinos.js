...
function selecionarQuatroReinosRecentes(){EVENTOS_REINOS.filter(r=>r.aberturaEm).slice().sort((a,b)=>new Date(b.aberturaEm)-new Date(a.aberturaEm)||Number(b.id)-Number(a.id)).slice(0,4).forEach(r=>EVENTO_REINOS_SELECTED.add(Number(r.id)));renderReinosEvento();}
