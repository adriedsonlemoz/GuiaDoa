...
const commonSource = {
  tipo:'screenshots_usuario', data:'2026-08-21',
  descricao:'Telas da Campanha/Grodz e relatórios de batalha enviados pelo usuário.', verificado:true,
};
...
const zeroLossGuide = (nivel) => ({
  ...
  observacoes:'Configuração informada pelo usuário como suficiente do nível 1 ao 9 sem perdas. Partes de armadura podem se repetir entre os ataques.',
  fonte:{ tipo:'usuario', descricao:'Configuração confirmada pelo usuário para Campanha/Grodz.' },
  i18n:{ 'en-US':{
    ...
    observacoes:'The user reports this setup as sufficient for levels 1–9 with zero losses. Armor parts may repeat across attacks.',
  } },
});

const level10Guides = [
  ['magmassauros','Magmassauros','Magmassaurs'],
  ['hoplita','Hoplita','Hoplite'],
  ['ogros-granito','Ogros de Granito','Granite Ogres'],
].map(([code,pt,en]) => ({
  codigo:`grodz-10-${code}`,
  titulo:`${pt} — Nv. 10`,
  resumo:'Alternativa para conta mais evoluída. No nível 10, perdas são esperadas.',
  status:'validacao', resultado:'possiveis_perdas', tropaPrincipal:pt, quantidade:5000,
  passos:['Selecione o dragão da armadura desejada.','Envie a marcha e considere as perdas como parte do custo do nível final.'],
  observacoes:'5.000 é uma referência conservadora sugerida pelo usuário; a quantidade mínima ainda precisa ser refinada com novos relatórios. Não existe configuração garantida sem perdas no momento.',
  fonte:{ tipo:'usuario', descricao:'Estimativa de ataque do usuário para o nível 10.' },
  i18n:{ 'en-US':{
    titulo:`${en} — Lv. 10`, resumo:'Alternative for a more developed account. Losses are expected at level 10.', tropaPrincipal:en,
    passos:['Select the dragon for the armor you want.','Send the march and treat losses as part of the final-level cost.'],
    observacoes:'5,000 is a conservative user estimate; the minimum amount still needs refinement from additional battle reports. There is currently no guaranteed zero-loss setup.',
  } },
}));
...
export const GRODZ_SEED = [
  {
    ...
    grodz:{ composicaoStatus:'confirmado', recomendacaoJogo:official(t('Milicianos',5,'Militiamen','milicianos')), dialogos:[
  ...
  },
  {
    ...
    grodz:{ composicaoStatus:'confirmado', recomendacaoJogo:official(t('Milicianos',40,'Militiamen','milicianos')), dialogos:[
  ...
  },
  {
    ...
    grodz:{ composicaoStatus:'confirmado', recomendacaoJogo:official(t('Alabardeiros',300,'Halberdiers','alabardeiros')), dialogos:[
  ...
  },
  {
    ...
    grodz:{ composicaoStatus:'confirmado', recomendacaoJogo:official(t('Minotauros',300,'Minotaurs','minotauros'),t('Arqueiros',200,'Longbowmen','arqueiros')), dialogos:[
  ...
  },
  {
    ...
    grodz:{ composicaoStatus:'confirmado', recomendacaoJogo:official(t('Dragões de Ataque Rápido',50,'Swift Strike Dragons','dragoes-de-ataque-rapido')), dialogos:[
  ...
  },
  {
    ...
    grodz:{ composicaoStatus:'confirmado', recomendacaoJogo:official(t('Dragões de Ataque Rápido',800,'Swift Strike Dragons','dragoes-de-ataque-rapido')), dialogos:[
  ...
  },
  {
    ...
    grodz:{ composicaoStatus:'confirmado', recomendacaoJogo:official(t('Dragões de Combate',150,'Battle Dragons','dragoes-de-combate')), dialogos:[
  ...
  },
  {
    ...
    tropas:[t('Dragões de …',500,'Dragons of …')], guiasAtaque:[zeroLossGuide(8)],
    grodz:{ composicaoStatus:'parcial', observacaoComposicao:'O relatório mostra “Dragões de …”; o tipo exato está truncado e não foi inferido.', recomendacaoJogo:official(t('Dragões de Combate',700,'Battle Dragons','dragoes-de-combate')), dialogos:[
    ...
    ], i18n:{'en-US':{observacaoComposicao:'The report shows “Dragons of …”; the exact type is truncated and was not inferred.'}} }, fonte:commonSource, i18n:{'en-US':{nome:'Grodz Field — Lv. 8'}},
  },
  {
    slug:'grodz-9', categoria:'grodz', nivel:9, ordem:9, nome:'Campo de Grodz — Nv. 9', ativo:true,
    tropas:[t('Dragões de …',600,'Dragons of …'),t('Gigantes',260,'Giants','gigantes')], guiasAtaque:[zeroLossGuide(9)],
    grodz:{ composicaoStatus:'parcial', observacaoComposicao:'O relatório mostra “Dragões de …”; o tipo exato está truncado e não foi inferido.', recomendacaoJogo:official(t('Gigantes',100,'Giants','gigantes'),t('Espelhos de Fogo',100,'Fire Mirrors','espelhos-de-fogo')), dialogos:[
    ...
    ], i18n:{'en-US':{observacaoComposicao:'The report shows “Dragons of …”; the exact type is truncated and was not inferred.'}} }, fonte:commonSource, i18n:{'en-US':{nome:'Grodz Field — Lv. 9'}},
  },
  {
    ...
    grodz:{ composicaoStatus:'pendente', observacaoComposicao:'O conjunto enviado confirma a tela final e a recompensa, mas não contém um relatório com a composição inimiga completa do nível 10.', recomendacaoJogo:[], dialogos:[
    ...
    ], i18n:{'en-US':{observacaoComposicao:'The supplied set confirms the final mission screen and reward, but does not include a battle report with the full level 10 enemy composition.'}} },
