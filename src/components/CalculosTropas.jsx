  ...
  return (
    <div className="max-w-2xl mx-auto pb-6">
      <Modal open={sim.confirmDialog.open} onClose={() => sim.setConfirmDialog(current => ({ ...current, open: false }))} maxWidth={300}>
        <div className="p-4 text-center"><p className="font-nunito font-black text-sm m-0 mb-1" style={{ color: C.ERROR }}>{sim.confirmDialog.title}</p><p className="font-nunito text-sm m-0 mb-4" style={{ color: C.TEXT_SECONDARY }}>{sim.confirmDialog.text}</p><div className="flex gap-2 justify-center"><button className="btn-ghost flex-1" onClick={() => sim.setConfirmDialog(current => ({ ...current, open: false }))}>Cancelar</button><button className="btn-danger flex-1" onClick={() => { sim.confirmDialog.acao?.(); sim.setConfirmDialog(current => ({ ...current, open: false })); }}>Confirmar</button></div></div>
      </Modal>
      <div className="mb-2"><GameHeader title="Simulador de Batalha" /></div>
      <button className="btn-ghost btn-sm mb-2.5" onClick={sim.solicitarSaida}>← Voltar ao Catálogo</button>
