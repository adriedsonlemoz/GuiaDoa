  ...
  return (
    <div className="tw-card mb-3" style={{ animation: 'reveal-up 0.4s 0.08s ease both' }}>
      <div className="flex items-center justify-between px-4" style={{ background: 'linear-gradient(135deg,#1C3A5E,#2A4C72)', borderBottom: '1px solid rgba(200,168,74,0.3)', minHeight: 36 }}>
        <span className="font-cinzel font-bold uppercase" style={{ fontSize: '0.58rem', color: 'rgba(200,168,74,0.7)', letterSpacing: '3px' }}>◆ {t('profile.hq')} ◆</span>
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          <button onClick={onLanguage} className="flex items-center justify-center rounded-md" style={{ width: 28, height: 28, background: 'transparent', cursor: 'pointer', border: '1px solid rgba(200,168,74,0.28)', color: 'rgba(248,242,224,0.55)', fontSize: '0.85rem' }} title={t('language.title')}>🌐</button>
          <button onClick={onEdit} className="flex items-center justify-center rounded-md" style={{ width: 28, height: 28, background: 'transparent', cursor: 'pointer', border: '1px solid rgba(200,168,74,0.28)', color: 'rgba(248,242,224,0.45)', fontSize: '0.9rem' }} title={t('home.perfil.editar')}>✎</button>
        </div>
      </div>
      <div className="flex items-center gap-4" style={{ background: C.BG_CARD_TOP, padding: '14px 16px' }}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex gap-1.5 mt-1.5 flex-wrap">
            <span className="font-nunito font-bold" style={{ fontSize: '0.68rem', padding: '2px 8px', borderRadius: 5, border: '1px solid rgba(200,168,74,0.35)', color: C.TEXT_SECONDARY, background: 'rgba(184,150,90,0.1)' }}>{t('profile.realm')}: {profile.reino}</span>
            {playerId && <button onClick={copiarId} className="font-nunito font-bold" style={{ fontSize: '0.68rem', padding: '2px 8px', borderRadius: 5, border: `1px solid ${idCopiado ? 'rgba(90,180,90,0.6)' : 'rgba(200,168,74,0.35)'}`, color: idCopiado ? '#5AB45A' : C.TEXT_SECONDARY, background: idCopiado ? 'rgba(90,180,90,0.12)' : 'rgba(184,150,90,0.1)', cursor: 'pointer', transition: 'all 0.2s', lineHeight: 1, display: 'flex', alignItems: 'center', gap: 4 }} title={t('profile.copy_id_title')}>{idCopiado ? <>✓ {t('profile.id_copied')}</> : <>📋 ID: {playerId}</>}</button>}
          </div>
        </div>
        ...
        <div className="text-right shrink-0">
          ...
          <p className="font-nunito font-black uppercase tracking-widest m-0" style={{ fontSize: '0.52rem', color: C.TEXT_MUTED, marginTop: 5 }}>{t('profile.server_utc')}</p>
        </div>
      </div>
    </div>
  );
