    ...
    <div className="tw-card mb-2">
      ...
      <div className="flex items-center justify-between px-3 py-2" style={{ background: C.BG_SECONDARY, borderTop: `1px solid ${C.BORDER_SOFT}` }}>
        ...
        <div className="flex gap-1.5">
          {isEditing
            ? <button className="btn-navy btn-sm" onClick={() => onRequestAction('save')}>⚔ Travar</button>
            : <button className="btn-ghost btn-sm" onClick={onEdit}>✏ Editar</button>}
          <button className="btn-danger btn-sm" onClick={() => onRequestAction('clear')}>🗑</button>
        </div>
      </div>
    </div>
  );
...
export function IlhasLimites({ metricas }) {
  const cards = [
    { label: 'CID. PRINC', val: metricas.cidPrinc, max: LIMITES.cidadePrincipal, icon: '🏙' },
    { label: 'SÍTIO PRINC', val: metricas.sitPrinc, max: metricas.limiteSipioPrinc, icon: '🌾' },
    { label: 'CID. ÁGUA', val: metricas.cidAgua, max: LIMITES.cidadeAgua, icon: '🌊' },
    { label: 'SÍTIO ÁGUA', val: metricas.sitAgua, max: LIMITES.sitioAgua, icon: '🔮' },
  ];
  return (
