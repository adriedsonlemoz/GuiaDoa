    ...
    <>
      <Modal open={promptAberto} onClose={() => setPromptAberto(false)} maxWidth={320}>
        <div className="p-4 text-center">
          ...
          <p className="font-cinzel font-bold text-sm tracking-wide uppercase m-0 mb-2" style={{ color: C.WARNING }}>{t('levels.update_title')}</p>
          <p className="font-nunito font-semibold text-sm leading-relaxed m-0 mb-4" style={{ color: C.TEXT_SECONDARY }}>
            {t('levels.update_question')}
          </p>
          <div className="flex gap-2">
            <button className="btn-ghost flex-1" onClick={() => setPromptAberto(false)}>{t('common.no')}</button>
            <button className="btn-success flex-1" onClick={onAtualizar}>{t('levels.update_yes')}</button>
          </div>
        </div>
      </Modal>
      ...
      <Modal open={resultadoDialog.open} onClose={() => setResultadoDialog(current => ({ ...current, open: false }))} maxWidth={320}>
        <div className="p-4 text-center">
          ...
          <p className="font-nunito font-semibold text-xs m-0 mb-3" style={{ color: C.TEXT_MUTED }}>{t('levels.report_local')}</p>
          <button className={resultadoDialog.tipo === 'success' ? 'btn-success w-full' : 'btn-gold w-full'} onClick={() => setResultadoDialog(current => ({ ...current, open: false }))}>{t('common.continue')}</button>
        </div>
      </Modal>
    </>
  );
