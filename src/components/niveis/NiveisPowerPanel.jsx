...
export default function NiveisPowerPanel(props) {
  ...
  const stats = [
    { label: 'Próximo Nível', value: proximaMeta ? `Nv. ${proximaMeta[0]}` : atingiuMax ? 'MAX' : '—', sub: proximaMeta && poderAtualText ? formatNumber(faltamParaMeta) : null, color: C.ACCENT_DEEP },
    { label: 'Marco', value: proximoMarco ? `Nv. ${proximoMarco[0]}` : atingiuMax ? 'MAX' : '—', sub: proximoMarco && poderAtualText ? formatNumber(faltamParaMarco) : null, color: C.POWER },
    { label: 'Total Níveis', value: carregando ? '…' : String(totalNiveis), sub: null, color: C.BLUE },
  ];
  return (
    <div className="flex gap-2 mb-3">
      <div className="tw-card flex-1 p-3">
        <p className="font-nunito font-bold text-[0.62rem] tracking-widest uppercase m-0 mb-1.5" style={{ color: C.TEXT_MUTED }}>Poder Anterior</p>
        <input className="tw-input text-center font-mono mb-3" placeholder="Ex: 500.000" value={poderAntigoText} onChange={handleInputAntigo} inputMode="numeric" />
        <div className="gold-stripe mb-3 opacity-30" />
        <div className="flex items-center justify-between mb-1.5">
          <p className="font-nunito font-bold text-[0.62rem] tracking-widest uppercase m-0" style={{ color: C.ACCENT_DEEP }}>Poder Atual</p>
        ...
        <div className="flex gap-1.5">
          <input ref={inputRef} className="tw-input text-center font-mono flex-1" placeholder="Digite o seu poder…" value={poderAtualText} onChange={handleInputPower} inputMode="numeric" />
          <button className={isDirty ? 'btn-success btn-sm shrink-0' : 'btn-ghost btn-sm shrink-0'} onClick={handleSave} disabled={!isDirty}>Salvar</button>
        </div>
        {isDirty && <p className="font-nunito font-bold text-[0.65rem] mt-1.5 m-0" style={{ color: C.WARNING }}>⚠️ Alterações não salvas</p>}
      </div>
