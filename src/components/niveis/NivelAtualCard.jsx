  ...
  return (
    <div className="tw-card mb-3 overflow-hidden" style={{ background: `linear-gradient(135deg, ${C.BG_HEADER} 0%, #0f2540 100%)`, border: `2px solid ${C.BORDER}` }}>
      <div className="flex items-center justify-between px-4 pt-4 pb-2">
        <div>
          <p className="font-nunito font-bold text-[0.6rem] uppercase tracking-widest m-0 mb-0.5" style={{ color: `${C.ACCENT}99` }}>Nível Atual</p>
          {carregando
          ...
          {!carregando && nivelExato > 0 && <p className="font-nunito font-semibold text-xs m-0 mt-1" style={{ color: `${C.ACCENT}88` }}>de {maxNivel} níveis</p>}
        </div>
      ...
      {poderAtualText && !atingiuMax && (
        <div className="px-4 pb-3">
          <div className="flex justify-between mb-1">
            <span className="font-nunito font-semibold text-[0.6rem]" style={{ color: `${C.ACCENT}88` }}>Nível {nivelExato}</span>
            <span className="font-nunito font-semibold text-[0.6rem]" style={{ color: `${C.ACCENT}88` }}>Nível {proximaMeta?.[0] ?? '—'}</span>
          </div>
      ...
      {poderAtualText && atingiuMax && <div className="px-4 pb-3 text-center"><p className="font-cinzel font-bold text-sm m-0" style={{ color: C.ACCENT }}>🏆 Nível Máximo Atingido!</p></div>}
    </div>
  );
}
