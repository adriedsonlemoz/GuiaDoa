  ...
  if (!tropa) {
    return (
      <button onClick={onSelecionar} style={{
        ...
        <div style={{ fontSize:'.72rem', fontWeight:750, marginTop:5 }}>{t('troops.slot',{number:index+1})}</div>
      </button>
    );
  }
  ...
  return (
    <div style={{
      ...
      <button onClick={onRemover} aria-label={`${t('common.delete')} ${nome}`} style={{
        ...
        fontSize:'.72rem', cursor:'pointer', display:'grid', placeItems:'center',
      }}>✕</button>
      <div style={{ fontSize:'1.45rem', lineHeight:1, textAlign:'center', marginTop:3 }}>{getIcone(tropa.nome)}</div>
      <div style={{ marginTop:6, fontSize:'.74rem', fontWeight:780, color:'#2E342F', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', textAlign:'center', padding:'0 10px' }}>{nome}</div>
      <div style={{ marginTop:3, fontSize:'.72rem', fontWeight:700, color:tipo.color, textAlign:'center', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{tipo.label}</div>
      <div style={{ marginTop:7, textAlign:'center' }}>
        <strong style={{ display:'block', color:'#315B56', fontSize:'.82rem' }}>{tropa.poder ? fmtFull(tropa.poder, locale) : '—'}</strong>
        <span style={{ color:'#7B8377', fontSize:'.72rem', fontWeight:800, letterSpacing:'.05em' }}>{t('common.power').toUpperCase()}</span>
      </div>
    </div>
  );
}
