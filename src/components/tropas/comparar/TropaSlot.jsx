  ...
  if (!tropa) {
    return (
      <button onClick={onSelecionar} style={{
        ...
        <p className="font-nunito font-bold" style={{ fontSize: '0.62rem', color: C.TEXT_MUTED, margin: 0, letterSpacing: '0.5px' }}>{t('troops.slot',{number:index+1})}</p>
      </button>
    );
  }
  ...
  return (
    <div style={{ flex: 1, minWidth: 0, padding: 8, borderRadius: 10, border: `2px solid ${cor}40`, borderTop: `3px solid ${cor}`, background: `linear-gradient(180deg,${cor}0A,transparent)`, position: 'relative' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: 6 }}>
        ...
        <p className="font-nunito font-bold m-0" style={{ fontSize: '0.66rem', color: C.TEXT_PRIMARY, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', lineHeight: 1.2, paddingRight: 12 }}>{nome}</p>
      ...
      <div style={{ textAlign: 'center' }}>
        ...
        <p className="font-nunito m-0" style={{ fontSize: '0.5rem', color: C.TEXT_FAINT, letterSpacing: '1px' }}>{t('common.power').toUpperCase()}</p>
      </div>
    </div>
  );
}
