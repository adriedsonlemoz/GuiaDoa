...
const SlotComparar = ({ tropa, label, side, onSelect }) => {
  ...
  <button onClick={() => onSelect(side)} className="flex-1 p-3 text-center cursor-pointer rounded-lg transition-all" style={{ border: tropa ? `2px solid ${C.ACCENT_HOVER}` : `2px dashed ${C.BORDER}`, background: tropa ? 'rgba(200,148,10,0.06)' : 'transparent' }}>
    ...
    <p className="font-nunito font-black text-[0.68rem] leading-tight m-0" style={{ color: tropa ? C.ACCENT : C.BORDER }}>{tropa ? content(tropa, 'nome') : t('troops.choose')}</p>
      ...
      {(tropaA || tropaB) ? (
        <div className="tw-card overflow-hidden"><GameHeader title={t('troops.attribute_comparison')} fontSize="0.82rem" /><div className="p-3">
          ...
          {ATTRS_COMPARAR.map(attr => {
            ...
            return (
              <div key={attr.id} className="mb-3"><p className="text-center font-nunito font-black text-[0.6rem] tracking-widest m-0 mb-1" style={{ color: C.TEXT_SECONDARY }}>{attr.icon} {attr.labelKey ? t(attr.labelKey) : attr.label}</p><div className="grid gap-1 items-center" style={{ gridTemplateColumns: '1fr 28px 1fr' }}>
