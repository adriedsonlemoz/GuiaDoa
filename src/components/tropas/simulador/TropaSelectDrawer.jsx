  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex flex-col" style={{ background: C.BG_MAIN }}>
      <div className="flex items-center gap-3 px-3 py-2.5 sticky top-0 z-10" style={{ background: C.BG_CARD_TOP, borderBottom: `2px solid ${C.BORDER}` }}>
        ...
        <p className="font-nunito font-black text-[0.85rem] tracking-wide flex-1 m-0" style={{ color: C.ACCENT }}>{t('troops.select')}</p>
        <span className="font-nunito text-xs" style={{ color: C.TEXT_SECONDARY }}>{t('troops.unit_count_short',{count:tropas.length})}</span>
      </div>
      <div className="px-3 py-2" style={{ background: C.BG_CARD, borderBottom: `1px solid ${C.BORDER_SOFT}` }}><input className="tw-input" placeholder={t('troops.search')} value={busca} onChange={event => setBusca(event.target.value)} autoFocus /></div>
