...
test('carga completa representa dados atualizados', () => {
  assert.equal(calcularSyncStatus({ ok: 3, total: 3 }), 'ok');
  assert.equal(descreverSyncStatus('ok', true).label, 'Dados atualizados');
});

test('carga parcial é explicitamente incompleta, sem fallback de cache', () => {
  assert.equal(calcularSyncStatus({ ok: 2, total: 3 }), 'parcial');
  assert.equal(descreverSyncStatus('parcial', true).label, 'Atualização parcial');
});
